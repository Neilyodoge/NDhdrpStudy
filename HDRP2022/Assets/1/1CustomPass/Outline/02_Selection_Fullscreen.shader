Shader "02_Selection/Fullscreen"
{
    properties
    {
        ...
        _OutlineDepthFalloff("Outline Depth Falloff", Range(0.0001, 100)) = 0.05
    }
    ...
    float4 FullScreenPass(Varyings varyings) : SV_Target
    {
        ...
        float d01 = Linear01Depth(d, _ZBufferParams);
        float dEye = LinearEyeDepth(d, _ZBufferParams);
        ...
        float obj = c.a;
        //123
        float scaledOutlineWidth = _OutlineWidth / (1.0 + saturate(posInput.linearDepth.x) * _OutlineDepthFalloff);
        ...
        {
            outline =  max( SampleCustomColor( posInput.positionNDC + uvOffsetPerPixel * _OutlineWidth * offsets[i] * scaledOutlineWidth), outline );//s  - dEye
           // outline =  max( SampleCustomColor( posInput.positionNDC + uvOffsetPerPixel * _OutlineWidth * offsets[i] ), outline );//s  - dEye
        }

        float4 o = float4(0,0,0,0);
        o = lerp(o, _OuterColor * float4(outline.rgb, 1), outline.a);
        o = float4(step(c,0.5).xxx ,1);
